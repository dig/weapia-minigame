    ...
    public boolean onCommand(CommandSender commandSender, Optional<AbstractPlayer> abstractPlayerOptional, String[] args) {
        if (args.length == 1 && isInteger(args[0])) {
            abstractPlayerOptional.ifPresent(abstractPlayer -> {
                ...
                survivalPlayer.setCoins(Integer.parseInt(args[0]));
                commandSender.sendMessage(String.format(Constants.ECONOMY_BALANCE_SET, args[0], survivalPlayer.getUsername()));
            });
            return true;
        }
